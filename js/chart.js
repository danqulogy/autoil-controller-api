...
(function ($, document) {
    var chart = $.fn.chart = function (data, options, hoverCb) {
        if (document.createElement("canvas").getContext) {
            this.each(function () {
                opts = $.extend(true, {}, chart.defaults, options)
                chart.graphers.call(this, data, opts, hoverCb)
            });
        }
    ...
    var u = {
        ...
        deltaHours: function (min, max) {
            var oneHour = 60 * 60 * 1000; // minutes*seconds*milliseconds
            return Math.round(Math.abs((min - max) / (oneHour)));
        },
        addZero: function (i) {
    ...
    chart.graphers =
        function (elements, opts, cb) {
            ...
            if (opts.labels.x.show) {
                ...
                hours = u.deltaHours(x.min, x.max)
                if (hours <= e) {
                    skip = 1;
                } else {
                    skip = ((hours / e)).toFixed();
                }
                var labelWidth = (width - x.marginLeft - x.margin) / hours;
                range = x.max - x.min;
                add = range / hours;
                x.labels = []
